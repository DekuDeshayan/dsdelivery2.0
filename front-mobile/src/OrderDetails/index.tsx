...
export default function OrderDetails({route}: Props) {
  ...
  const handleConfirmDelivery = () => {
    confirmDelivery(order.id).then(() => {
      Alert.alert(
        t("Sucesso"), 
        t(`Entrega número ${order.id} confirmada`)
      );
      navigation.navigate('Orders');
    }).catch(() => {
      Alert.alert(
        t("Erro"), 
        t(`Não foi possível confirmar a entrega do pedido número ${order.id}`)
      );
    });
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <RectButton style={styles.button} onPress={handleStartNavigation}>
        <Text style={styles.buttonText}>{t('Iniciar Navegação')}</Text>
      </RectButton>
      <RectButton style={styles.button} onPress={handleConfirmDelivery}>
        <Text style={styles.buttonText}>{t('Confirmar Entrega')}</Text>
      </RectButton>
